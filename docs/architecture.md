...
| `alphabrief_core/read_contracts` | shared versioned read contracts for API and CLI | M13-W01 起：`VersionedReadEnvelope` 是 14 个 read surface（instruments/prices/candles/news/sentiment/committee/risk/orders/trades/positions/cycles/scheduler/alerts/observation）的唯一 versioned schema（`READ_SCHEMA_VERSION="read-v1"`）——UTC 强制、稳定 id + id 排序、provenance（source/data_version/retrieved_at）、freshness verdict（fresh/stale/unknown）、cursor pagination（has_more⇔next_cursor）、显式 complete/empty/partial state（与 items 一致性校验）；`normalize_read_payload` 给出 API/CLI 共用的 canonical payload（sort_keys + 紧凑分隔符）；`ReadErrorResponse` + 四个 typed error builder（unknown_filter/malformed_cursor/invalid_identifier/unavailable_source）绝不携带 fake 或截断数据（REQ-UI-001、REQ-PLAT-008/009） |
| `alphabrief_core/write_contracts` | safe and idempotent operator write contracts | M13-W02 起：`OPERATOR_MUTATIONS` 为 REQ-UI-010 的 closed 7-mutation set（pause/resume research、freeze/unfreeze paper execution、cancel practice order、reduce/close practice exposure），每个 mutation 有 approved endpoint 与 broker-payload key allowlist；`WriteContractGate` 是纯函数 fail-before-invocation gate——host（仅 practice host，live 拒绝）、account（cross-account 拒绝）、endpoint（arbitrary_endpoint 拒绝）、payload（arbitrary_broker_payload 拒绝）、state version（stale_version 拒绝）、unsupported mutation 全在 provider 调用前拒绝；`MutationRequest` 强制 idempotency_key/actor/correlation_id，`MutationAuditLog` append-only 记录每次 accepted/rejected（同 key replay 返回同一 audit 与 result，审计 ID 确定性）；gate 无任何 provider/HTTP 引用（REQ-UI-002、REQ-UI-010、REQ-PLAT-009） |
| `alphabrief_api/routes/operational` | operational portfolio resources from runtime stores | M13-W03 起：`GET /api/v1/operational/portfolio` 从共享 `PaperStore`/`BrokerReconStore`/`InstrumentCatalogStore` 读取——cash/NAV/realized/unrealized（total-cash-realized）、gross/net exposure、positions、pending orders（仅非终态）、fills、margin_used（由 catalog margin_rate 推导）、category attribution（taxonomy 分类）、observed_at/snapshot_id（store 时间戳）；store 无法提供的字段（如 financing）显式 null 绝不 fake；`GET /api/v1/operational/equity` 返回持久化 equity 时序；全部只读、逐请求打开共享 store、无 route-local 状态（REQ-EXEC-010、REQ-UI-007） |
| `alphabrief_api/routes/trace` | end-to-end cycle traceability | M13-W03 起：`GET /api/v1/trace/cycles/{cycle_id}` 通过稳定 ID 解析完整证据链——cycle record（`AiTradingStore`）→ evidence（snapshot_fingerprint/key_evidence）→ committee transcript（votes）→ proposal 或显式 no_trade → intents（intent_id + audit resolution）→ risk rules（risk_decision_id/intent_id）→ OANDA transactions（order ledger）→ reconciliation（recon snapshots）；缺失 → 404，未解析链接显式 not_found，绝不伪造（REQ-EXEC-010、REQ-UI-006） |
| `alphabrief_cli/contracts` | machine-readable CLI contracts | M13-W04 起：文档化确定性 exit codes（0 success / 1 internal / 2 validation / 3 empty / 4 partial / 5 conflict / 6 unavailable / 7 frozen）+ typed error 子类；`emit_json` 输出 stable compact JSON（sort_keys、无 prompt）；`emit_error` 写 `{"error_code","exit_code","message"}` 到 stderr 并确定性退出；`normalize_payload`/`equivalent_normalized_payloads` 定义 API-backed 与 local 读的 canonical 等价；`read_local_or_api`（API up 走 API，失败 fallback 到 local 只读——绝不获取 writer lease，与 scheduler/API writer 零冲突）（REQ-UI-001/002/010） |
| `alphabrief_api/openapi_contract` | deterministic locked OpenAPI contract | M13-W05 起：`build_deterministic_openapi` 生成字节级确定的 OpenAPI（sort_keys + compact）并附加 `x-alphabrief-contract` extension——声明 14 read domains、7 approved operator mutations + approved endpoints、typed error model、cursor fields、freshness fields、`Idempotency-Key`/`X-Correlation-ID` header；`verify_openapi_contract` fail-closed（缺任一声明 → issues）；`scan_for_sensitive_values` 扫描 schema 文本中的 bearer/api-key/authorization；API-CLI parity 由 CLI→OpenAPI path 映射表 + normalized payload 等价证明（REQ-UI-001/002/006、REQ-PLAT-009） |
| `alphabrief_core/observability` | structured observability and runtime readiness truth | M15-W01 起：`StructuredLogRecord`/`MetricRecord`/`ComponentHealth`/`HealthRegistry` 覆盖 11 个关键子系统（ingestion/news/models/scheduler/cycle/risk/oanda/reconciliation/backup/api/electron）——每组件发布 health/readiness/heartbeat/latency/success/failure/freshness，缺失 truth → unknown 且 not ready（绝不假设 healthy）；`correlation_chain` 用 correlation ID 串联 cycle/evidence/model/intent/risk/order/transaction/reconciliation/alert/backup 十类记录；`redact_observable` 强制 scrub token/authorization/完整 account ID/model-sensitive/未许可新闻全文/可配置 secret pattern，`account_id_hash` 不可逆展示（REQ-OPS-001、REQ-OPS-002） |
| `alphabrief_core/alerting` | error taxonomy and durable alert lifecycle |
| `alphabrief_core/request_policy` | external request deadlines, retry budgets, concurrency limits |
| `alphabrief_core/preflight` | deterministic engineering preflight scopes | M15-W04 起：`run_preflight` 在单一 schema 中运行三 scope（engineering_readiness 6 check / oanda_observation 14 check / final_release 4 check）；OANDA-observation scope 验证 practice host、secret presence（仅 boolean 绝不披露值）、account/catalog/data/content/ModelGateway/risk/backup/scheduler lease/reconciliation/alerts/frozen build/safety gates；缺 truth → fail-closed 显式 detail（REQ-OPS-007） |
| `alphabrief_core/observation_controller` | practice E2E path + observation evidence controller |
| `alphabrief_core/recovery` | graceful shutdown, crash recovery, bounded soak |
| `alphabrief_core/security_gates` | security gates before observation | M15-W06 起：`run_security_gates` 校验 7 gate（dependency_integrity/supply_chain_policy/secret_scan/artifact_scrub/network_allowlist/reference_boundary/static_security_rules），缺 truth fail-closed；`scan_files_for_secrets` 检出完整 account id；`scan_network_allowlist` 只允许 OANDA practice 两 host（live/other-broker 引用即报）；`pip check`/ruff/mypy 入 static gate（REQ-OPS-008） |
| `alphabrief_core/runbook_rehearsal` | prompt-injection invariants + runbook rehearsal |
| M15 里程碑 | operations observability/security/readiness closure | W01-W06 全部落地（observability/error taxonomy+alerts/request budgets/preflight+observation controller/shutdown+recovery+soak/security gates+rehearsal）；W07 Engineering Readiness Gate 聚合证明通过——targeted 212 + integration 289 + full pytest 3101 passed（19 pre-existing M08-W03 time-bomb 分类见 M10-W03）+ pip check clean + ruff/mypy clean（501）+ acceptance 11/11；`engineering_readiness_verdict`（M01..M15 DONE + tree clean + practice-only）标记 build 冻结；真实 OANDA practice E2E 与 T7 外部证据 PENDING（记录 blocker 不伪造）；M15 里程碑 → DONE | M15-W06 起：`verify_injection_invariants` 证明注入 fixture 不能改变 system instructions/risk limits/broker tools/provider routing/execution state/evidence citations 六个受保护面；`run_rehearsal` 完成 8 步非生产彩排（day_zero/daily_record/no_trade_day/weekly_gate/incident_reset/restart/restore/final_report），`counts_as_observation` 恒 False——彩排时间绝不记为真实观察；`apps/cli/observation_commands.py` 提供 `observation rehearse --all-drills --compact`（REQ-OPS-002/008） | M15-W05 起：`SHUTDOWN_SEQUENCE` 8 步（freeze→stop_new_cycle→resolve_uncertain_submit→sync→reconcile→checkpoint→backup→lease_release）+ `SHUTDOWN_BUDGET_S=30`；`run_recovery_drill` 对 12 个声明边界（startup..complete）产出 resumed/frozen 确定性裁决，缺 truth fail-closed frozen；`run_soak` 校验 7 项 invariants（heartbeats/writer_ownership/memory_budget/descriptor_budget/projection_equality/reconciliation_truth/backup_integrity），缺 truth not preserved；`apps/cli/operations_commands.py` 提供 `operations recovery-drill`/`operations soak` 两个 runtime 命令（stable compact JSON）（REQ-OPS-001、REQ-OPS-006） | M15-W04 起：`validate_e2e_sequence` 只接受正式 7 步路径（proposal→OrderIntent→persisted RiskDecision→submit→transaction→cleanup→reconciliation），direct_broker_submit/in_memory_fill/live_execution/simulated_fallback 一律拒绝；`ObservationSupervisor` 单 leader、NDJSON 持久化、重启恢复 next-run 状态、`observation_day_index` 从真实 UTC 日历推导 Day 0..30（越界 None 绝不伪造）、daily gate 自动调用、BLOCKED_EXTERNAL/WAITING_EXTERNAL 无证据记录（REQ-OPS-002/007） | M15-W03 起：`REQUEST_BUDGETS` 为 7 类 external request family（oanda_rest/oanda_stream/market_data/content/model/alert/backup）配置 connect/read/total/cycle budget + max_attempts + jittered backoff + max_concurrency；`backoff_seconds` 确定性 jitter（seed 绑定）；`retry_allowed` 有界且仅 retryable 错误类；`submit_outcome_action`——unknown/timeout submit outcome → query_and_reconcile，绝不 blind retry（OANDA 连接限制）；`classify_timeout` 产生完整 scrubbed telemetry，超时任务自包含隔离（不阻塞 heartbeat/reconciliation/backup/risk freeze/其他调度工作）（REQ-OPS-003、REQ-OPS-005） | M15-W02 起：`classify_error` 把 8 类操作失败（auth/validation/broker_reject/rate_limit/transient/protocol/data_quality/safety）确定性映射为 retryable/severity/freeze_execution/no_trade/escalate，未知类 fail-closed 为 safety blocker；`AlertStore` 以 NDJSON append-only 持久化 alert（severity/dedupe_key/first+last occurrence/count/acknowledged/escalated/resolved/incident_link/scrubbed evidence），重启恢复；同 dedupe_key 重复事件只递增 count（有界 storm），ack/escalate/resolve 为显式状态机，`sink_failure` 对外部 sink 失败零影响（绝不删除/解决本地 alert）（REQ-OPS-003、REQ-OPS-004） |
...
| M12 里程碑 | strategy/backtest research-quality closure | W01-W06 全部落地（safe DSL/五族+admission/portfolio+OANDA semantics/evaluation/reporting/leakage+overfitting gates）；W07 聚合证明通过——REQ-STRAT-001..008 全 trace、targeted 207 + integration 65 + full pytest 2568 passed（19 pre-existing M08-W03 time-bomb 分类见 M10-W03）+ ruff/mypy clean（430）+ acceptance 11/11；arbitrary-code 不可执行与零 broker 副作用由 DSL 禁例表与 gym/Kronos advisory-boundary 测试证明；T7 practice backtest evidence PENDING（CODE_COMPLETE，无 runtime 依赖）；M12 里程碑 → DONE |
| M13 里程碑 | API/CLI operator-contract closure | W01-W06 全部落地（read contracts/write contracts/operational+portfolio resources/cycle trace/CLI machine-readable contracts/deterministic OpenAPI contract/API-CLI parity）；W06 聚合证明通过——REQ-UI-001/002/006/007/010 全 trace、targeted 123 + integration 182 + static gate 10 + full pytest 2711 passed（19 pre-existing M08-W03 time-bomb 分类见 M10-W03）+ ruff/mypy clean（445）+ acceptance 11/11；static gate 证明无 arbitrary broker proxy/route-local state/offline-success placeholder/live control/undocumented mutation/sensitive OpenAPI example；无 T7 runtime 依赖；M13 里程碑 → DONE |
| `alphabrief_api/dashboard/design_system` | Soft 5/5/5 design token system + brand audit | M14-W01 起：`DESIGN_TOKENS` 单一机器可读 token 源（color_light/color_dark/typography/spacing/radius/elevation/interaction/motion，含 prefers-reduced-motion 行为）；`static/design-tokens.css` 由其确定性生成（light 默认 + prefers-color-scheme dark + reduced-motion 块）；`validate_design_tokens` 自动化校验（WCAG AA 对比度、无 emoji/em dash、无 gradient、无动画库依赖）；`BRAND_AUDIT` 对 9 个 legacy dashboard route 记录 retained assets/defects/replacement（纯数据，route 处理器零改动）（REQ-UI-003、REQ-UI-008） |
...
## 13. Operator Handoff Surface (M17-W02)

M17-W02 起，`alphabrief_core.recovery` 新增三个 operations 契约（无新 module
boundary、无 state ownership 变化，仅新增确定性检查面）：

- `FRESH_INSTALL_STEPS`（5 项）：locked deps install、empty data dir init、
  migrate、local readiness、no untracked-source dependency——干净 checkout
  只能依赖已提交源码 + 外部注入的 runtime secrets（REQ-OPS-006）。
- `OPERATOR_RUNBOOK_STEPS`（11 项）：credential injection、startup、preflight、
  scheduler control、freeze、safe shutdown、backup、isolated restore、
  restart、reconciliation、blocker inspection——`secrets_exposed` 恒 False，
  runbook 永不打印或持久化凭证（REQ-OPS-007）。
- `MAINTENANCE_POLICIES`（6 项）：backup retention、restore cadence、
  dependency review、incident retention、evidence retention、practice-account
  reset——`no_live_procedure` 恒 True，长期维护无任何 live-trading 流程
  （REQ-OBS-007）。
